...
void SmtpClient::setEmailId(const int &emailId)
{
    this->emailId = emailId;
}

const int &SmtpClient::getEmailId() const
{
    return this->emailId;
}
...
bool SmtpClient::connectToHost()
{
    ...
    {
        emit smtpError(ConnectionTimeoutError, emailId);
        return false;
    }
    ...
    try
    {
        ...
        if (responseCode != 220)
        {
            emit smtpError(ServerError, emailId);
            return false;
        }
...
bool SmtpClient::login(const QString &user, const QString &password, AuthMethod method)
{
    try {
        if (method == AuthPlain)
        {
            ...
            if (responseCode != 235)
            {
                emit smtpError(AuthenticationFailedError, emailId);
                return false;
            }
        }
        else if (method == AuthLogin)
        {
            ...
            waitForResponse();
            if (responseCode != 334) { emit smtpError(AuthenticationFailedError, emailId);
                return false; }
            ...
            // Wait for 334
            waitForResponse();
            if (responseCode != 334) { emit smtpError(AuthenticationFailedError, emailId); return false; }
...
void SmtpClient::waitForResponse() throw (ResponseTimeoutException)
{
    do {
        ...
        {
            emit smtpError(ResponseTimeoutError, emailId);
        ...
        while (socket->canReadLine()) {
            ...
            if (responseCode / 100 == 5)
                emit smtpError(ClientError, emailId);
